...
//	seed (Math.floor (Math.random () * mask));
	// no result shown:
	seed (1253723019);

	for (var structure of Object.values (currentElectionPreset))
		for (var office of structure)
			for (var candidate of office.candidates) {
				candidate.yes = Math.floor (Math.random () * 50);
				candidate.no  = Math.floor (Math.random () * 30);

				candidate.candidate.female  = Math.random () < 1 / 2.;
				candidate.candidate.diverse = Math.random () < 1 / 4.;
			}
